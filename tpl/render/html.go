...
func HtmlBaseRenderResponse(w http.ResponseWriter, rs tpl.RenderState) error {
	return HtmlBaseRender(w, rs)
}

func HtmlBaseRender(w io.Writer, rs tpl.RenderState) (err error) {
...
func writeDocStart(w io.Writer, rs tpl.RenderState) {
	w.Write([]byte(`<!DOCTYPE html><html lang="en">`))
}

func writeHead(w io.Writer, rs tpl.RenderState) {
	w.Write([]byte(`<head>`))
...
func writeBody(w io.Writer, rs tpl.RenderState) {
	w.Write([]byte(`<body>`))
	// Body content

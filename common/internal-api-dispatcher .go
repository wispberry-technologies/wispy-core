...
type APIDispatcher interface {
	...
	Call(method, path string, body []byte, headers map[string]string) (*APIResponse, error)
}
...
func (d *RouterAPIDispatcher) Call(method, path string, body []byte, headers map[string]string) (*APIResponse, error) {
	startTime := time.Now()

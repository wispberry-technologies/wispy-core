...
type CSSBuilder interface {
	AddSection(comment string) CSSBuilder
	AddProperty(name, value string) CSSBuilder
	AddPropertyFromMap(prefix string, props map[string]string) CSSBuilder
	AddPropertyFromNestedMap(prefix string, props map[string]map[string]string) CSSBuilder
	AddRule(selector, properties string) CSSBuilder
	AddLayer(layerName string, content func(CSSBuilder)) CSSBuilder
	AddBlock(content string) CSSBuilder
	Close(block string)
	String() string
}
...
func (b *cssBuilder) Close(block string) {
